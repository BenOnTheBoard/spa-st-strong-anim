...
class SPAST:
    ...
    def instance_generator_no_ties(self):
        ...
        for i in range(
            self.tpc - self.projects
        ):  # range(9 - 8) = range(1) = 1 iteration. Okay!
        ...
        # -----------------------------------------------------------------------------------------------------------------------------------------
        self.sp = {
            "s" + str(i): [[]] for i in range(1, self.students + 1)
        }  # stores randomly selected projects
        ...
        self.lp = {
            "l" + str(i): [0, [], [], 0, 0] for i in range(1, self.lecturers + 1)
        }  # we assign l1:[p1], l2:[p2], ..., l30:[p30]
        ...
        for lecturer in self.lp:
            ...
            for _ in range(number_of_projects):
                ...
                projects_copy.remove(
                    p
                )  # I did this to avoid picking the same project 2x. This could also be achieved by shuffling and popping?
                self.plc[p][1] = (
                    lecturer  # take note of the lecturer who is offering the project
                )
                self.lp[lecturer][1].append(p)
                self.lp[lecturer][2].extend(
                    self.plc[p][2]
                )  # keep track of students who have chosen this project for the lecturer
                self.lp[lecturer][4] += self.plc[p][
                    0
                ]  # increment the total project capacity for each lecturer
                if (
                    self.plc[p][0] > self.lp[lecturer][3]
                ):  # keep track of the project with the highest capacity
        ...
        for lecturer in self.lp:
            ...
            random.shuffle(
                self.lp[lecturer][2]
            )  # this line shuffles the final preference list for each lecturer. Hence ordered.
            self.lp[lecturer][0] = random.randint(
                self.lp[lecturer][3], self.lp[lecturer][4]
            )  # capacity for each lecturer
